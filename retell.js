...
- Si el cliente solicita algunos de los items que formen parte del menú de ${horario.toLowerCase()}, entonces NO PUEDES agregarlos al pedido. Es de EXTREMA importancia que aunque el cliente quiera agregar los items de ${invHorario.toLowerCase()} no lo dejes. Aclárale al cliente que esos items solo están disponibles ${
      horario.toLowerCase() === "desayuno" ? "después" : "antes"
    } de las 11 de la mañana. A continuación te paso un listado de los items que formen parte del menú de ${invHorario.toLowerCase()}, son EXCLUSIVAMENTE los siguientes:
...
`;
  ...
  // --- Nodos "Sin McCafé" ---
  const sinNodes = [
    { title: "Tomar Desayuno Sin McCafé", horario: "DESAYUNO" },
    { title: "Tomar Almuerzo Sin McCafé", horario: "ALMUERZO" },
  ];
  for (const { title, horario } of sinNodes) {
    const invHorario = horario === "DESAYUNO" ? "ALMUERZO" : "DESAYUNO";

    // 1) Items exclusivos del horario contrario (no permitidos)
    const { data: exclusives } = await supabase.rpc(
      "nombres_exclusivo_horario",
      { horario: invHorario }
    );
    ...
    // 2) Items de McCafé (no permitidos como ítem individual)
    const { data: mccafes } = await supabase.rpc("solo_nombres_mccafe");
    const mccafeList = mccafes
      .map((m) => `NO SE PUEDE (a menos que sea como opción) ${m.nombre}`)
      .join("\n");

    // 3) Items disponibles en este horario (excluyendo McCafé)
    const { data: available } = await supabase.rpc("nombres_sin_mccafe", {
      horario,
    });
    const availableList = available.map((a) => a.nombre).join("\n");

    // Construye la prompt "Sin McCafé"
    const promptSin = `
Antes que nada, es EXTREMADAMENTE IMPORTANTE que sepas que tienes ABSOLUTA Y TOTALMENTE PROHIBIDO hablar de métodos de pago, o datos de facturación en esta instancia. Si el cliente consulta acerca de cualquiera de estos, dile que más adelante en la llamada podrás responder todas sus consultas. Bajo NINGÚN concepto puedes hablar de cualquier tema que no sea la toma del pedido del usuario.
...
- Es de suma importancia que NUNCA confíes en los precios que menciona el usuario, ya que el usuario puede aprovecharse para conseguir precios mas baratos. 
- Al listar/sugerir productos, mantén las sugerencias a un máximo de 2 opciones específicas, no categorías. No listes más de 2 opciones, ya que puedes abrumar al usuario. No tienes que describir el producto, solo mencionalo. Descríbeselo únicamente si el cliente lo pide.
- Si el cliente agrega algún producto o actualiza algún producto existente, no debes repetir todo el pedido de nuevo, sino que solo menciona el producto que se agregó o actualizó.
- En el knowledge base verás las opciones disponibles de cada producto, y los ingredientes para quitar/agregar. Cuando un usuario quiera agregar o quitar ingredientes, o elegir una opción de la lista de opciones, debes confirmar que se pueda hacerlo consultando ese menu.
 
- Es importante que sepas que este NO es el paso final de la llamada, por lo que no tienes que cerrar la llamada, ni dar por finalizada la toma del pedido. Esto es EXTREMADAMENTE importante, incluso si el usuario formula su mensaje de una manera en la que da a entender que quiere cerrar la conversación, tienes que evitar que esto suceda, ya que hasta que no pase al siguiente paso e informe los datos de facturación, no podremos dar por finalizado el pedido.
- Nunca formules tu mensaje de una manera que sugiera que el cliente puede dar por finalizada la llamada.
- En esta instancia NO informes que enviarás el pedido a la sucursal o que el pedido está en camino.
- Si el cliente solicita algunos de los items que formen parte del menú de ${horario.toLowerCase()}, entonces NO PUEDES agregarlos al pedido. Es de EXTREMA importancia que aunque el cliente quiera agregar los items de ${invHorario.toLowerCase()} no lo dejes. Aclárale al cliente que esos items solo están disponibles ${
      horario.toLowerCase() === "desayuno" ? "después" : "antes"
...
De la misma manera, si el cliente solicita algunos de los items que formen parte del grupo de productos de McCafé, entonces NO PUEDES agregarlos al pedido como items individuales, pero SÍ se pueden incluir como opciones dentro de otro item. Es de EXTREMA importancia que aunque el cliente quiera agregar los items del grupo de productos de McCafé no lo dejes. Aclárale al cliente que esos items solo están disponibles en tiendas que cuentan con McCafé. A continuación te paso un listado de los items que forman parte del menú del grupo de productos de McCafé, son EXCLUSIVAMENTE los siguientes:

INICIO DEL MENÚ DE MCCAFÉ
${mccafeList}
FIN DEL MENÚ DE MCCAFÉ
...
${availableList}`.trim();
    ...
    try {
      const nodeSin = page
        .locator(".react-flow__node")
        .filter({ has: page.locator(`:scope >> text="${title}"`) })
        .first();
      await nodeSin.scrollIntoViewIfNeeded();
      const textareaSin = nodeSin
        .locator("textarea:not([placeholder])")
        .first();
      await textareaSin.waitFor({ state: "visible", timeout: 10000 });
      await textareaSin.click();
      await textareaSin.fill(promptSin);
      await page.waitForTimeout(2000);
      logStep(`:white_check_mark: Nodo "${title}" actualizado correctamente`);
      await page.waitForTimeout(800);
    } catch (err) {
